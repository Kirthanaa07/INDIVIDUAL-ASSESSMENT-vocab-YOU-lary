...
const myVocabularyFilterActions = (array) => {
  let btnString = '<div class="d-flex justify-content-center gap-3"><button class="filter-btn-css btn btn-success btn-lg mb-4" id="filter-my-btn--all">All</button>';

  array.forEach((item) => {
    // "--" here separates the id into 2 parts because 'filter-btn,categoryID' are 1 id.
    btnString += `<button class="filter-btn-css btn btn-success btn-lg mb-4" id="filter-my-btn--${item.category_id}">${item.name}</button>`;
  });
  btnString += '</div>';
